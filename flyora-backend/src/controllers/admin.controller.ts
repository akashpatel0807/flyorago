...
export const updateAdminCredentials = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    // Update credentials for the default admin (assuming single admin)
    await query(`
      UPDATE admins 
      SET username = $1, password_hash = $2, updated_at = NOW()
      WHERE id = 'admin-id-1' OR username = 'admin'
    `, [newUsername.trim(), hash]);
    ...
    res.status(500).json({
      success: false,
      message: 'Internal server error updating admin credentials',
      error: error.message,
    });
  }
};
